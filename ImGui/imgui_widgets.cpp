...
bool ImGui::Checkbox(const char* label, bool* v)
{
   ImGuiWindow* window = GetCurrentWindow();
    ...
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 3.0f));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id))
    {
    ...
    RenderNavHighlight(total_bb, id);
    RenderFrame(check_bb.Min - ImVec2(1, 1), check_bb.Max + ImVec2(1, 1), GetColorU32(ImGuiCol_FrameBgActive), true, style.FrameRounding);
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
    if (*v)
        RenderFrame(check_bb.Min + ImVec2(4, 4), check_bb.Max - ImVec2(4, 4), GetColorU32(ImGuiCol_FrameBgActive, alpha), true, style.FrameRounding);
    ...
    if (mixed_value)
    {
        ...
        ImVec2 pad(ImMax(2.0f, IM_FLOOR(square_sz / 3.6f)), ImMax(2.0f, IM_FLOOR(square_sz / 3.6f)));
        window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad * 2, check_col, style.FrameRounding);
    }
    if (g.LogEnabled)
